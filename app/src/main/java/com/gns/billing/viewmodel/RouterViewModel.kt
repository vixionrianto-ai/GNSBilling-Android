...
class RouterViewModel : ViewModel() {
    ...
    private val _pppProfiles = MutableStateFlow<List<PppProfile>>(emptyList())
    val pppProfiles: StateFlow<List<PppProfile>> = _pppProfiles
    ...
    fun loadPppProfiles(routerId: Int) = viewModelScope.launch { try { _pppProfiles.value = repository.getPppProfile(routerId).data } catch (e: Exception) { _connectionMessage.value = e.localizedMessage ?: "Gagal memuat PPP Profile." } }
    ...
    fun createPppProfile(routerId: Int, name: String, local: String, remote: String, rate: String, onlyOne: String, onResult: (Boolean, String) -> Unit) = viewModelScope.launch { try { val r = repository.createProfile(routerId, name, local, remote, rate, onlyOne); onResult(r.success, r.message ?: "Operasi selesai.") } catch (e: Exception) { onResult(false, e.localizedMessage ?: "Gagal membuat PPP Profile.") } }
    fun updatePppProfile(routerId: Int, id: String, name: String, local: String, remote: String, rate: String, onlyOne: String, onResult: (Boolean, String) -> Unit) = viewModelScope.launch { try { val r = repository.updateProfile(routerId, id, name, local, remote, rate, onlyOne); onResult(r.success, r.message ?: "Operasi selesai.") } catch (e: Exception) { onResult(false, e.localizedMessage ?: "Gagal memperbarui PPP Profile.") } }
