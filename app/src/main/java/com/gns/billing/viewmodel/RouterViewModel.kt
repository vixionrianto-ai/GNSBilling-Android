...
class RouterViewModel : ViewModel() {
    ...
    fun loadRouter() = viewModelScope.launch {
        _loading.value = true
        try {
            _router.value = repository.getRouter().data
        } catch (e: Exception) {
            _connectionMessage.value = e.localizedMessage ?: "Gagal memuat router."
        } finally {
    ...
    fun loadSecrets(routerId: Int, onResult: (List<PppSecret>) -> Unit) = viewModelScope.launch {
        try {
    ...
    fun deleteSecret(routerId: Int, secret: String, onResult: (Boolean, String) -> Unit) =
        operation(onResult) { repository.deleteSecret(routerId, secret) }

    fun enableSecret(routerId: Int, secret: String, onResult: (Boolean, String) -> Unit) =
        operation(onResult) { repository.enableSecret(routerId, secret) }

    fun disableSecret(routerId: Int, secret: String, onResult: (Boolean, String) -> Unit) =
        operation(onResult) { repository.disableSecret(routerId, secret) }

    private fun operation(
        onResult: (Boolean, String) -> Unit,
        action: suspend () -> MessageResponse
    ) = viewModelScope.launch {
        try {
            val r = action()
            onResult(r.success, r.message ?: "Operasi selesai.")
